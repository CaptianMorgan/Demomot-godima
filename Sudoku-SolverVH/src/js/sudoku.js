...
function changeDifficulty(numberMax,numberMin,LowerBoundByRowsCols,Name)
{
    ...
    document.getElementById("difficultyList").innerHTML = difficultyName + '<span class="mui-caret"></span>'}
...
function verification(elements)
{
    ...
    if(numberFalse.length > 0) {
        ...
        var session = "";
        session = document.getElementById("sessionPseudo").value;
        if(session != "")
        {
 ...
 */
function verificationTab(elements)
{
    //tableau de jeu
...
function saveScoreOverlay(endTime) {
    ...
    modalEl.innerHTML= '<form action="../php/saveScore.php" method="post" class="mui-form">Vous avez gagné en ' + endTime +' ms en mode ' + difficultyName + '. Le score vas être enregisté si vous appuier sur OK<br><input id="score" name="score" type="text" style="visibility: hidden" value="'+endTime+'"><input id="difficulty" name="difficulty" type="text" style="visibility: hidden" value="'+difficultyName+'"><button type="submit" class="mui-btn mui-btn--raised">Ok</button> </form>';
...
function resolution()
{
    ...
    if(verificationSudokuBeforeSolv(elements)) {
...
function verificationSudokuBeforeSolv(elements)
{
    ...
    {
        if(document.getElementById(numberFalse[i]).innerHTML != "")
        {
            return false;
        }
    }
